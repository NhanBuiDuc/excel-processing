<script>
	...
	export let filter = (text) => {
		...
		return options.reduce((a, o) => {
			...
			if (o.options) {
				const options = o.options.filter((o) => o.name.toLowerCase().includes(sanitized));

				if (options.length) {
					match = { ...o, options };
				}
			} else if (o.name.toLowerCase().includes(sanitized)) {
				match = o;
			}
	...
	function hideList() {
		...
		if (selectedOption) {
			...
			// inputElement.value = selectedOption.value;
			// inputElement.text = selectedOption.text;
		}
